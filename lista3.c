...
for(int cont =1; cont<= 100; cont++){
 ...
 void exemploEnquanto(){
    int opcao =-1;
    while (opcao !=0){
        printf("Digite uma opçao (0 oara sair):");
        scanf("%d",&opcao);
...
void exemploPrimeiroMilhao(){
    float aporte=1000;
    float saldo =0;
    float rentabilidade = 0.02;
    int prazo =0;
    while (saldo <1000000){
        saldo=aporte + saldo + saldo*rentabilidade;
        prazo++;
    }
    printf("Saldo final após %d meses : %.2f",prazo,aporte);
}
...
void q4(){
    ...
    for(pessoas = 1;pessoas<20;pessoas++){
